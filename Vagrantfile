...
$script = <<-SCRIPT
ALGORITHM=allium
URL=stratum+tcp://allium.eu.mine.zpool.ca
PORT=6433
WALLET_ADDRESS=1MSsQfdLFhAbKFUX6YHkub5emhMegC6ogJ
PASSWORD="c=BTC"
echo connecting to $URL:$PORT with $PASSWORD password and $WALLET_ADDRESS wallet address and $ALGORITHM algorithm..
echo "cpuminer -a $ALGORITHM -o $URL:$PORT -u $WALLET_ADDRESS -p $PASSWORD -B"
cpuminer -a $ALGORITHM -o $URL:$PORT -u $WALLET_ADDRESS -p $PASSWORD -B
SCRIPT
